    ...
    def forward(self, input):
        ...
        if self.buffer_num > 0 and self.training and input.requires_grad:
            ...
            dmudw = torch.autograd.grad(
                cur_mu, self.prev_module_weight, self.ones, retain_graph=True
            )[0]
            dmeanx2dw = torch.autograd.grad(
                cur_meanx2, self.prev_module_weight, self.ones, retain_graph=True
            )[0]
            mu_all = torch.stack(
                [cur_mu,]
                + [
                    tmp_mu
                    + (self.rho * tmp_d * (self.prev_module_weight.data - tmp_w))
                    .sum(1)
                    .sum(1)
                    .sum(1)
            ...
            self.pre_meanx2 = [cur_meanx2.detach(),] + self.pre_meanx2[
                : (self.buffer_num - 1)
            ]
            self.pre_dmudw = [dmudw.detach(),] + self.pre_dmudw[: (self.buffer_num - 1)]
            self.pre_dmeanx2dw = [dmeanx2dw.detach(),] + self.pre_dmeanx2dw[
                : (self.buffer_num - 1)
            ]
            tmp_weight = torch.zeros_like(self.prev_module_weight.data)
            tmp_weight.copy_(self.prev_module_weight.data)
            self.pre_weight = [tmp_weight.detach(),] + self.pre_weight[
                : (self.buffer_num - 1)
            ]
        else:
            x = y
